...
import type { NewArrivalsConfig } from '@/types/home-config';
        ...
        <div className="flex gap-4 sm:gap-5">
          {products.map((p, i) => (
            <Link
              key={p.id}
              href={productHref(p)}
              className="basis-[180px] sm:basis-[240px] md:basis-[260px] shrink-0 snap-start block"
            >
              <ProductCard variant="minimal" product={toUiProduct(p, i)} />
            </Link>
          ))}
        </div>
      </div>
