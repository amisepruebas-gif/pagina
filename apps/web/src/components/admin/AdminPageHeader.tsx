  ...
  return (
    <div className="px-6 py-5 border-b border-border bg-surface">
        ...
        >
          {breadcrumb.map((b, i) => (
            <Fragment key={i}>
              ...
              {b.href ? (
                <Link href={b.href} className="hover:text-text transition">
                  {b.label}
                </Link>
              ) : (
      ...
      <div className="flex justify-between items-end gap-3 flex-wrap">
        <div>
          <h1 className="font-display font-bold leading-tight tracking-[-0.02em] text-xl sm:text-2xl lg:text-[clamp(22px,3vw,28px)]">
            {title}
          </h1>
          {description && (
            <p className="mt-1.5 text-text-muted text-sm max-w-2xl">
              {description}
            </p>
          )}
        </div>
