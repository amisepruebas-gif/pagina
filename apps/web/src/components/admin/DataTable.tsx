...
import { Button, IconButton, Icon, type IconName } from '@/components/ui';
...
/** DataTable — tabla reutilizable del admin con estado vacío y paginación. */
export function DataTable<T extends { id?: string | number }>({
  ...
  if (!rows || rows.length === 0) {
    return (
      <div className="bg-surface border border-border rounded-lg py-14 px-5 text-center">
        ...
        {empty?.cta && (
          <div className="mt-4">
            <Button size="sm" leadingIcon="plus" onClick={empty.onCta}>
              {empty.cta}
            </Button>
          </div>
        )}
  ...
  return (
    <div className="bg-surface border border-border rounded-lg overflow-hidden">
            ...
            <tr className="bg-surface-2">
              {columns.map((c) => (
                <th
                  ...
                  className="px-3.5 py-2.5 font-display font-semibold text-[11px] tracking-[0.06em] uppercase text-text-soft border-b border-border"
                >
                  {c.label}
                </th>
              ))}
      ...
      {pagination && (
        <div className="px-4 py-2.5 border-t border-border flex justify-between items-center flex-wrap gap-2.5 text-[13px] text-text-muted">
          <span>
