...
/** FormSection — bloque de formulario: título a la izquierda, campos a la derecha. */
export function FormSection({ title, description, children }: FormSectionProps) {
  return (
    <div className="grid gap-3 lg:gap-8 grid-cols-1 lg:grid-cols-[260px_1fr] py-6 border-t border-border">
      <div className="lg:pt-1.5">
        <h3 className="font-display font-bold text-[15px] tracking-[-0.015em]">
          {title}
        </h3>
        {description && (
...
/** FlagRow — fila con toggle + título + subtítulo. */
  ...
  return (
    <label className="flex items-center gap-3.5 py-3 cursor-pointer border-t border-border first:border-t-0">
      ...
      <div className="flex-1">
        ...
        {subtitle && (
          <div className="mt-0.5 text-xs text-text-soft">{subtitle}</div>
        )}
      </div>
    </label>
  );
