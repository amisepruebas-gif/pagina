...
export interface ProductCardProps {
  ...
  /** Handler al agregar al carrito (solo quick-actions) */
...
function FavButton({
  ...
  return (
    <button
      ...
      className={cn(
        "rounded-pill bg-white/90 backdrop-blur shadow-sm inline-flex items-center justify-center",
  ...
  return (
    ...
    >
      ...
      <div className="pt-3.5 px-1 flex flex-col gap-2">
        <span className="font-mono text-[11px] tracking-[0.08em] uppercase text-text-soft">{p.brand}</span>
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-5 text-white flex flex-col gap-2">
        <span className="font-mono text-[11px] tracking-[0.08em] uppercase opacity-85">{p.brand}</span>
...
function Discount({ product: p, onToggleFav, onSelect, className }: ProductCardProps) {
  ...
  return (
    <article
      onClick={() => onSelect?.(p)}
      className={cn(
        "relative bg-surface rounded-xl border-2 border-secondary p-3.5 shadow-secondary",
        "transition duration-base ease-out",
        className,
      )}
    >
      {/* circular stamp */}
      <div className="absolute -top-4 -right-2.5 z-10 size-[78px] rounded-full bg-gradient-to-br from-secondary to-[#FF8FB1]
      ...
      <div className="relative">
        ...
        <div className="absolute bottom-2.5 left-2.5">
          <FavButton active={fav} onClick={() => { setFav(!fav); onToggleFav?.(p, !fav); }} size={32} />
        </div>
      </div>
      ...
      <div className="pt-3.5 px-1 flex flex-col gap-2">
        ...
        <div className="flex items-end gap-2.5 mt-1">
          <span className="font-display font-extrabold text-[28px] text-secondary-600 tracking-[-0.03em] leading-none">
            {fmt(p.price)}
          </span>
          {p.oldPrice && (
            <div className="flex flex-col mb-0.5">
              <span className="text-[13px] text-text-soft line-through">{fmt(p.oldPrice)}</span>
              <span className="text-[11px] text-secondary-600 font-semibold">Ahorras {fmt(p.oldPrice - p.price)}</span>
            </div>
          )}
        </div>
        ...
        <div className="mt-2">
          <div className="flex justify-between text-[11px] mb-1">
            <span className="text-secondary-600 font-semibold">¡Solo quedan {stock}!</span>
            <span className="text-text-soft">Vendidos {sold}</span>
          </div>
          <div className="h-1.5 rounded-full bg-surface-2 overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-secondary to-accent" style={{ width: `${sold}%` }} />
          </div>
        </div>
      </div>
    </article>
        ...
        <div className="absolute inset-x-3 bottom-3 flex flex-col gap-1.5
          ...
          {p.sizes && p.sizes.length > 0 && (
            <div className="flex gap-1.5 p-1.5 bg-white/90 backdrop-blur rounded-pill shadow-sm justify-center">
              {p.sizes.map((s) => (
                <button key={s} type="button" onClick={(e) => { e.stopPropagation(); onAddToCart?.(p, s); }}
          ...
          <div className="flex gap-1.5">
            <Button leadingIcon={added ? "check" : "cart"} fullWidth onClick={add}>
    ...
    >
      <div className="relative overflow-hidden rounded-lg">
        ...
        <button
          ...
          aria-label="Favorito"
          className={cn(
            "absolute top-2 right-2 size-8 inline-flex items-center justify-center",
            "transition duration-fast ease-out",
            fav ? "text-secondary-600" : "text-black/55",
          )}
        >
