...
export function CatalogHeader({ title, description, count }: CatalogHeaderProps) {
  return (
    <div className="py-2">
      ...
      {description && (
        <p className="mt-3 text-text-muted text-sm sm:text-base max-w-xl">{description}</p>
      )}
 ...
 *
 * @example <SearchHeader query="audífonos" count={42} />
 */
export function SearchHeader({ query, count }: SearchHeaderProps) {
  return (
    <div className="py-2">
      ...
      <h1 className="mt-3 font-display font-bold leading-tight tracking-[-0.03em]
                     text-3xl sm:text-4xl lg:text-[clamp(28px,4.5vw,44px)]
                     flex flex-wrap items-baseline gap-3">
        <span>Resultados para</span>
        <span className="bg-brand-grad bg-clip-text text-transparent">"{query}"</span>
      </h1>
      <p className="mt-2 text-text-soft text-[13px]">
        {count.toLocaleString("es-MX")} {count === 1 ? "resultado" : "resultados"}
      </p>
