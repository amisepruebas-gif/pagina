 ...
 *
 * @example <StatCard label="Ingresos" value="$684,219" change={12.4} icon="bolt" />
 */
export function StatCard({ label, value, change, icon, tone = "brand" }: StatCardProps) {
  ...
  return (
    <div className="bg-surface border border-border rounded-lg p-[18px] flex flex-col gap-3">
      <div className="flex justify-between items-start">
        ...
        <span className={cn("size-9 rounded-sm inline-flex items-center justify-center shrink-0", TONES[tone])}>
      ...
      {change != null && (
        <div className="inline-flex items-center gap-1.5 text-xs">
          <span className={cn(
            ...
            positive ? "bg-success/[0.12] text-success" : "bg-error/[0.12] text-error",
          )}>
